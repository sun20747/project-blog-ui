...
export default function userwaitingconfirmed() {
  ...
  React.useEffect(() => {
    fetcherWithToken("http://127.0.0.1:3000/api/v1/admin/show_users_waiting", {
      method: "GET",
    })
      .then((json) => {
        setUsers(json.users);
  ...
  function al_delete(id, f_name, l_name) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetcherWithToken(
          `http://localhost:3000/api/v1/admin/deleteuserwarining/${id}`,
          {
            method: "DELETE",
          }
        )
  ...
  function al_confirm(id) {
    ...
    Toast.fire({
      ...
      fetcherWithToken(
        `http://localhost:3000/api/v1/admin/confirmeduser/${id}`,
        {
          method: "POST",
        }
      )
